function Piece(color, type, obj, square){
	...
	this.updateMoves = function(){
		...
		case Piece.PAWN:
						...
						if (this.square.row == 2){
							if (row == 3 || (row == 4 && squares[i][j - 1].piece == null))
								if (this.square.column == col)
									...
									if (Math.abs(this.square.column - col) == 1&& testSquare.piece != null && row == 3)
							...
							if (row == this.square.row + 1)
								if (this.square.column == col)
									if (testSquare.piece == null)
										this.available[i][j] = Piece.MOVE_ONLY;
									else
										this.available[i][j] = Piece.NO_MOVE;			
								else if (Math.abs(this.square.column - col) == 1
						...
						if (this.square.row == 7){						
							if (row == 6 || (row == 5 && squares[i][j + 1].piece == null))
								if (this.square.column == col)
									...
									if (Math.abs(this.square.column - col) == 1 && testSquare.piece != null && row == 6) 
										if (testSquare.piece.color != this.color)
											this.available[i][j] = Piece.CAPTURE;
										else
											this.available[i][j] = Piece.FRIENDLY_PIECE;
			...
			// En Passant
			if (this.epleft && this.square.column != 1){
				if (this.color == Piece.WHITE)
					this.available[this.square.column - 2][this.square.row] = Piece.EN_PASSANT;
				else
